...
# before World Cup
original <- read.csv("output_2026.csv")
## after Round 1 games
...
get_progression_probabiltiies <- function(df, n_sim, from_round) {
  ...
  for (i in 1:nrow(df)) {
    progression <- c(sum(df[i, c("R32", "R16", "QF", "SF", "RU", "W")]) / n_sim,
    		     sum(df[i, c("R16", "QF", "SF", "RU", "W")]) / n_sim,
  ...
  colnames(prog_df) <- c("Team", "R32", "R16", "QF", "SF", "F", "W")
  return(prog_df)
}
...
get_progression_prob_differences <- function(new_df, old_df) {
  ...
  for (i in 1:nrow(new_df)) {
    team <- new_df[i,]$Team
    new_probabilities <- new_df[new_df$Team==team, c("R32","R16", "QF", "SF", "F", "W")]
    old_probabilities <- old_df[old_df$Team==team, c("R32","R16", "QF", "SF", "F", "W")]
  ...
  colnames(prog_difference) <- c("Team", "R32", "R16", "QF", "SF", "F", "W")
