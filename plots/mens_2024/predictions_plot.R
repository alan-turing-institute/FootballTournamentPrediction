...
get_progression_probabilities <- function(df, n_sim, from_round) {
...
create_table_plot <- function(data_frame,
  ...
  full_table <- data_frame %>%
    ...
    gt() %>%
    tab_header(
      title = md("**Men's European Championships 2024**"),
      subtitle = subtitle) %>%
    cols_label(Team = "",
               logo = "") %>%
    cols_width(Team ~ 80,
    ...
    tab_source_note(
      source_note = md("The Alan Turing Institute (Nick Barlow, Jack Roberts, Ryan Chan)<br>Based on 10,000 simulations<br>Data: GitHub (martj42/international_results)<br>Flag Images: Flaticon.com")
    )
  if (!is.null(filename)) {
...
# original plot
original_prob <- get_progression_probabilities(original, 10000)
...
## after round 1 plots
#after_round_1_prob <- get_progression_probabilities(round_1, 100000)
